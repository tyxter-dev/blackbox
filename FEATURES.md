...
| Claude Code Agent SDK provider | Supported | `ClaudeCodeAgentProvider` | Supports injected `client=...` implementations and the optional production `claude-agent-sdk` wrapper for sessions, streaming events, workspace/tool events, approvals, artifacts, cancellation, and resume metadata. Forwards SDK options (including `setting_sources`, so `permissions={"setting_sources": ["project"]}` activates the workspace `.claude/` skills/subagents/commands) and filters unknown option keys to degrade gracefully across SDK version skew. |
