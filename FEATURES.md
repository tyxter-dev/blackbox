...
| Portable skill packs | Supported | `SkillSpec.from_directory(...)`, `runtime.run(..., skills=[...])` | Loads `SKILL.md` frontmatter/body, compiles active skills into prompt fragments, local tool refs, hosted/MCP tools, output specs, workspace requirements, and skill approval policy. `RuntimeConfig(overrides={"skills": [...]})` is supported; `SkillSpec.to_markdown()` exports documentation deterministically. |
...
| Workspace agent runtime grants | Supported | `permission_mode="allowlist_v1"`, `run_workspace_agent(...)` | Filters static/dynamic tools and checks fresh dispatch scopes/connectors in model runs and local sessions. Empty grants deny tools; default inherit is compatible. Managed agents and opaque native tool configuration fail explicitly; WebSearch read grants and client-hosted handlers are supported with documented limitations. |
| Workspace agent validation | Supported | `validate_workspace_agent(...)`, `ensure_valid_workspace_agent(...)`, `ValidationIssue` | Errors for unresolvable tool refs, unknown connectors/MCP servers, duplicates, bad schedule expressions/timezones, missing/unparseable local `SKILL.md`; warnings for calendar triggers, missing local skill sources, and models absent from the (snapshot) catalog. |
| SQLite workspace agent registry | Supported | `SQLiteWorkspaceAgentRegistry` | Durable `WorkspaceAgentRegistry`: every version kept, latest pointer for default resolution, visibility-filtered listing, publish/deprecate; same connection/lock/to-thread pattern as the other SQLite stores. |
| Workspace agent package on disk | Supported | `save_workspace_agent_package(...)`, `load_workspace_agent_package(...)`, `pack_workspace_agent_package(...)`, `unpack_workspace_agent_package(...)`, `install_workspace_agent_package(...)` | `agent.json` manifest + `instructions.md` + embedded `skills/<name>/` bundles; directories are diffable, archives are installable into any `WorkspaceAgentRegistry`; zip-slip guarded; newer `format_version` fails loudly. Typed `hosted_tools` entries become dicts on load (same fidelity as the dict round-trip). |
...
| Provider model catalog | Supported | `ProviderModel`, `ProviderModelCatalog`, `bundled_provider_model_catalog()` | Runtime seeds provider-native model identity metadata separately from pricing, including aliases, lifecycle, replacements, modalities, capacity hints, and per-row source provenance. The 2026-09-05 snapshot adds current OpenAI/Claude/Grok text models and retains retired identities for audit; Astra availability is account-dependent; unchanged Google rows retain their older retrieval date. |
| Model usage and cost accounting | Supported | `ModelUsage`, `ModelCatalog`, `ModelPricing`, `MarkupPolicy`, `bundled_provider_pricing()` | Provider adapters normalize usage, split cache read/cache creation tokens where available, count tool calls, preserve provider usage details, and estimate `provider_cost` from bundled or user provider-price catalogs while keeping `cost` as a compatibility alias. Applications can add billable pricing or markup policies for `billable` metadata. Bundled pricing reuses provider model aliases but remains the monetary catalog. |
...
| Claude Code Agent SDK provider | Supported | `ClaudeCodeAgentProvider` | Supports injected `client=...` implementations and the optional production `claude-agent-sdk` wrapper for sessions, streaming events, workspace/tool events, approvals, artifacts, cancellation, and resume metadata. Forwards SDK options (including `setting_sources`) and filters unknown option keys to degrade gracefully across SDK version skew. `run_workspace_agent` stages active skill bundles into workspace `.claude/skills/` and adds `setting_sources=["project"]` for Claude Code runs. |
| Claude Code subscription auth | Supported | `ClaudeCodeAgentProvider(auth=...)` | `auth="auto"` (default) uses API-key billing when `api_key`/`ANTHROPIC_API_KEY` is set, else falls back to a logged-in Claude Pro/Max subscription (`claude login` / `CLAUDE_CODE_OAUTH_TOKEN`). `auth="subscription"` forces OAuth and neutralizes any inherited `ANTHROPIC_API_KEY`; `auth="api_key"` requires a key. |
| Cloud agent providers | Partial | `OpenAICloudAgentProvider`, `ClaudeCodeAgentProvider`, `CodexAgentProvider`, `VertexAIAgentEngineProvider` | OpenAI Agents SDK, Claude Code, and Codex app-server are implemented; Vertex Agent Engine remains an honest stub. Codex uses the version-pinned optional SDK runtime and existing native subscription auth (never `OPENAI_API_KEY`), streams threads/turns/items, pauses command/file approvals, and emits file-change artifacts; Blackbox local tools and MCP specs are not mapped yet. |
...
| MCP server spec | Supported | `MCPServerSpec` | Server configuration model exists for `stdio`, `http`, `sse`, and `streamable_http`, including auth identity, protocol negotiation, allow/deny filters, timeout, cache, output-limit, and redaction controls. |
| MCP server authoring | Supported | `MCPServer` / `MCPToolError` | Local stdio MCP servers can be authored with decorators, optional Pydantic input models, schema generation, structured result helpers, and tool-error normalization compatible with `MCPConnector`. |
| MCP protocol compatibility matrix | Supported | `mcp_protocol_matrix()`, `SUPPORTED_MCP_PROTOCOL_VERSIONS` | Declares supported MCP spec versions and gates client negotiation through that matrix. |
| Remote MCP auth refresh | Supported | `OAuthBearerMCPAuthProvider`, `MCPAuthenticationError` | HTTP MCP transports retry once after 401/403 auth challenges, surface typed actionable auth failures, and keep tokens out of repr/error text. |
| MCP trust policy presets | Supported | `MCPTrustPolicyPresets` | Provides `local_only`, `enterprise_remote`, and `provider_native_allowed` presets for production trust posture defaults. |
| Local MCP dispatch connector | Supported | `MCPConnector` | Registers in-process MCP tools, starts managed stdio/HTTP transports, lists namespaced `mcp:<server>.<tool>` refs, gates calls through policy with server/tool/scope/risk metadata, normalizes MCP results, caches discovery by auth identity, invalidates on `listChanged`, and emits MCP lifecycle/cache/call events. |
...
| MCP toolset routing | Supported | `MCPToolset` / `runtime.run(..., toolsets=[...])` | Routes MCP servers to local dispatch or provider-native `RemoteMCP` based on mode, provider capability, transport, URL locality, auth, and policy; local runtime-managed toolsets reuse their identity-aware discovery cache across runs. |
...
| Production observability presets | Supported | `ObservabilityPreset.production(...)` | One runtime config switch enables OTLP trace export, OTLP/memory metrics, redacted JSONL event logging, and replay/diff trace links. |
...
| Workflow trace spans | Supported | `result.metadata["trace"]`, `trace_from_events(...)` | Runtime reconstructs `agent.run` rooted span trees and span events for model, tool, hosted tool, MCP, workspace, approval, artifact, cache, retry, handoff, guardrail, and eval events. |
...
| Standard production metrics | Supported | `metrics_from_trace(...)`, `OpenTelemetryMetricExporter` | Extracts first-event/token timing, model/tool/MCP/workspace latency, validation attempts, retries, cache hit rate, tokens, estimated cost, and provider error rate. |
