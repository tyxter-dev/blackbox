...
| Model usage and cost accounting | Supported | `ModelUsage`, `ModelCatalog`, `ModelPricing`, `MarkupPolicy`, `bundled_provider_pricing()` | Provider adapters normalize usage, split cache read/cache creation tokens where available, count tool calls, preserve provider usage details, and estimate `provider_cost` from bundled or user provider-price catalogs while keeping `cost` as a compatibility alias. Applications can add billable pricing or markup policies for `billable` metadata. |
