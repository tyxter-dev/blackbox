...
from agent_runtime.core.errors import ProviderExecutionError, WorkspaceError
from agent_runtime.core.events import EventTypes
from agent_runtime.workspaces.local import LocalWorkspaceProvider, _is_directory
...
class GitWorkspaceProvider(LocalWorkspaceProvider):
    ...
    async def open(self, spec: WorkspaceSpec) -> WorkspaceRef:
        ...
        root = Path(spec.root) if spec.root is not None else Path(tempfile.mkdtemp(prefix="agent-runtime-git-"))
