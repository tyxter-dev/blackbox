...
from blackbox.observability.traces import SpanKinds, Trace, TraceSpan
...
def _base_attributes(
    ...
    *,
    ...
    attrs: dict[str, MetricAttribute] = {"blackbox.trace_id": trace.id}
    if service_name:
...
def _span_attributes(
    ...
    attrs = dict(base)
    attrs["blackbox.span_id"] = span.span_id
    attrs["blackbox.span_kind"] = span.kind
    attrs["blackbox.span_name"] = span.name
...
def _event_attributes(
    ...
    if event.run_id is not None:
        attrs["blackbox.run_id"] = event.run_id
