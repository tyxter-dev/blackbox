...
from blackbox.tools.registry import ToolCallable, ToolDefinition
from blackbox.tools.results import ToolResult
from blackbox.workspaces.changes import ChangeType, CommandSpec, FileChange, Patch
from blackbox.workspaces.serialization import workspace_state_to_dict
from blackbox.workspaces.spec import WorkspaceRef
    ...
    return [
        register(
            ...
            scopes={
                ...
                "write_file": ["write"],
                "delete_file": ["delete"],
                "apply_patch": ["write", "delete"],
                "run_command": ["execute"],
                "snapshot": ["read"],
                "expose_port": ["execute"],
            }[name[len(prefix) + 1 :]],
            metadata={
                "workspace_operation": name[len(prefix) + 1 :],
                "ref": f"workspace:{name[len(prefix) + 1 :]}",
                "workspace_id": ref.id,
