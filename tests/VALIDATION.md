...
| 1.17 | ✅ | Model usage and cost accounting | `unit/test_model_accounting.py`, `integration/openai/test_cache_and_cost.py`, `integration/gemini/test_cache_and_cost.py`, `integration/xai/test_cost_tracking.py` | Provider usage payloads normalize into result metadata, split cache read/cache creation tokens where available, preserve provider details, and optional catalog pricing produces cost estimates without hard-coded prices. |
...
| 1.25 | ✅ | Native provider cache controls | `unit/test_model_request_controls.py`, `integration/openai/test_cache_and_cost.py`, `integration/anthropic/test_cache_and_cost.py`, `integration/gemini/test_cache_and_cost.py` | Runtime accepts typed cache controls and maps them to OpenAI/xAI prompt cache fields, Anthropic ephemeral block cache controls, and Gemini cached-content references. Live cache tests are network-gated and skip when provider credentials or cache resources are absent. |
