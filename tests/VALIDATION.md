...
| 1.14 | ✅ | Gemini GenerateContent adapter parity | `golden/gemini/test_generate_content_event_mapping.py`, `integration/gemini/test_generate_content_smoke.py` | Third real provider maps streaming events, preserves its terminal raw chunk and candidate-zero finish reason, without flattening to chat; live smoke is gated by `GOOGLE_API_KEY`. |
...
| 1.17 | ✅ | Model usage and cost accounting | `unit/core/test_model_accounting.py`, `unit/core/test_pricing_catalog.py`, `runtime/test_agent_session_run.py::test_agents_run_collects_session_result_with_output_and_artifacts`, `integration/openai/test_cache_and_cost.py`, `integration/gemini/test_cache_and_cost.py`, `integration/xai/test_cost_tracking.py` | Provider usage payloads normalize into result metadata, split cache read/cache creation tokens where available, count tool calls, preserve provider details, estimate provider API cost from bundled or user catalogs, keep `cost` as a compatibility alias, and support separate billable pricing through user billable catalogs or markup policies. |
...
| 1.22 | ✅ | MCP event and cache metadata | `golden/openai/test_responses_event_mapping.py::test_mcp_items_include_typed_run_item_data`, `golden/anthropic/test_messages_event_mapping.py::test_mcp_blocks_map_to_typed_run_items`, `unit/core/test_model_accounting.py::test_model_runtime_collects_mcp_cache_metadata`, `::test_blackbox_result_metadata_includes_mcp_cache_metadata` | MCP list/call/approval items become typed run items and result metadata exposes cacheable tool-list context IDs. |
...
| 1.25 | ✅ | Native provider cache controls and registry | `unit/providers/model_adapters/test_model_request_controls.py`, `unit/core/test_model_accounting.py`, `unit/core/test_provider_cache.py`, `runtime/test_cache_metadata.py`, `integration/openai/test_cache_and_cost.py`, `integration/anthropic/test_cache_and_cost.py`, `integration/gemini/test_cache_and_cost.py` | Runtime accepts typed cache controls, maps them to provider-native cache fields, reports cache-hit ratios based on read tokens with legacy combined-count fallback, persists cache registry records in memory or SQLite, supports invalidation/expired-entry eviction, and creates/deletes Gemini context caches where the provider API is available. Live cache tests are network-gated and skip when provider credentials or cache resources are absent. |
...
| 1.33 | ✅ | Provider model catalog | `unit/providers/test_model_catalog.py`, `unit/providers/test_bundled_model_catalog.py`, `unit/providers/model_adapters/test_current_model_controls.py`, `unit/core/test_pricing_catalog.py::test_agent_runtime_uses_bundled_model_aliases_for_pricing` | Runtime exposes provider-native model identity metadata separately from pricing, including aliases, lifecycle, replacements, modalities, capacity hints, and source provenance. Bundled pricing reuses these aliases without making pricing the model inventory. Current-model fixtures cover capacities, retirement provenance, standard/cache rates, native cache-write accounting and inclusive Claude cache totals/aggregate costs, Astra parameter restrictions, current OpenAI cache TTL mapping, adaptive and raw controls, hosted exclusions, and Fable 5.1 prefix/model replay rejection before SDK dispatch. Non-Fable replay retains its prior behavior. |
...
| 3.14 | ✅ | Portable skill packs | `unit/skills/test_skills.py`, `runtime/test_runtime_skills.py` | `SkillSpec` loads/exports `SKILL.md`, compiles deterministically into runtime primitives, and `runtime.run(..., skills=[...])` exposes skill tools and prompt fragments, including `RuntimeConfig` overrides. |
...
| 3A.6 | ✅ | Package validation | `unit/workspace_agents/test_validation.py` | Workspace-agent validation reports identity, tool, connector, schedule, model, and skill-source issues, including Windows absolute skill sources without treating external bundle URLs as local paths and local bundles missing/invalid `SKILL.md`. |
| 3A.7 | ✅ | Workspace-agent skill activation | `unit/workspace_agents/test_workspace_agents.py::test_run_workspace_agent_stages_skills_for_claude_code` | `run_workspace_agent` forwards model-backed package skills to `runtime.run` and stages Claude Code package skills into `.claude/skills/` with project setting sources enabled. |
...
| 4.13a | ✅ | Claude Code subscription auth | `runtime/test_claude_code_agent_provider.py::test_claude_code_auto_uses_subscription_without_api_key`, `::test_claude_code_subscription_neutralizes_inherited_api_key`, `::test_claude_code_auth_api_key_requires_key` | `auth="auto"` falls back to a logged-in Claude Pro/Max subscription when no API key is present; `auth="subscription"` neutralizes an inherited `ANTHROPIC_API_KEY`; `auth="api_key"` still requires a key. |
| 4.13b | ✅ | Claude Code `setting_sources` passthrough + option filtering | `runtime/test_claude_code_agent_provider.py::test_claude_code_forwards_setting_sources_for_project_skills`, `::test_claude_code_setting_sources_via_task_extra`, `::test_filter_options_kwargs_drops_unknown_keys_with_warning`, `::test_filter_options_kwargs_passes_through_var_keyword_classes` | `setting_sources` (from `spec.permissions` or `task.extra`) reaches `ClaudeAgentOptions` so the workspace `.claude/` skills/subagents/commands load; unknown option keys are dropped with a `RuntimeWarning` (SDK version skew) instead of raising `TypeError`. |
| 4.13c | ✅ | Claude Code turn boundaries, task budgets, and startup ownership | `runtime/test_claude_code_agent_provider.py::test_claude_code_forwards_task_budget_and_emits_turn_boundaries`, `::test_claude_code_turn_boundary_uses_agent_model_when_task_model_is_absent`, `::test_claude_code_turn_boundary_preserves_empty_task_model_override`, `::test_claude_code_rejects_unbounded_or_malformed_task_budget`, `::test_claude_code_startup_failure_reaps_and_forgets_provisional_owner`, `::test_claude_code_retains_uninspectable_failed_start_for_explicit_retry` | Each initial and follow-up SDK query emits `model.request.started` with the exact model selected by the same task-over-agent precedence as SDK options, plus a monotonic turn number; only a bounded exact `task_budget` reaches `ClaudeAgentOptions`; failed/cancelled startup reaps and forgets a confirmed child, while an uninspectable owner remains available through the narrow retry/confirmation callback instead of being silently lost. |
| 4.13d | ✅ | Codex app-server lifecycle and subscription boundary | `golden/codex/test_app_server_event_mapping.py`, `runtime/test_codex_agent_provider.py` | The pinned `openai-codex==0.147.0` runtime drives reviewed thread/turn/item mapping, command/file approval pauses, follow-up/steer, cancellation, file-change artifacts, and process-local event replay. Codex itself reuses and refreshes subscription authentication; Blackbox never reads or brokers credential material. |
